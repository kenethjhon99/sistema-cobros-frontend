...
export const EstudianteCuotas = () => {
  ...
  const handleSelect = async (id) => {
    ...
    try {
      ...
      console.log("Cuotas:", res.data);
      setCuotas(res.data);
    } catch (err) {
      console.error("Error obteniendo cuotas:", err);
    }
  };
  // Manejo de pago de cuota
  const [cuotaSeleccionada, setCuotaSeleccionada] = useState(null);
  const [pago, setPago] = useState({
    monto: "",
    observacion: "",
  });
  //si el estudiante no tiene cuotas asignadas, mostrar mensaje
  ...
  const pagarCuota = async () => {
    try {
      ...
      setCuotas(
        cuotas.map((c) =>
          c.id === cuotaSeleccionada.id ? { ...c, estado: "pagado" } : c
        )
      );
  ...
  return (
    <div>
      ...
      {estudianteId && cuotas.length === 0 && (
        <p style={{ marginTop: "20px" }}>
          El estudiante no tiene cuotas asignadas.
        </p>
      )}
        ...
        >
          ...
          >
            ...
            <input
              ...
              onChange={(e) =>
                setPago({ ...pago, observacion: e.target.value })
              }
            />
            <br />
